...
PROFILE_PATH = ROOT / "config" / "hp_stoplight.json"
...
def load_hp_stoplight_profile() -> dict:
    """Load the persisted HP stoplight profile with safe defaults."""
    try:
...
def save_hp_stoplight_profile(profile: dict) -> None:
    """Persist the complete HP stoplight profile."""
...
def classify_hp_frame(frame_rgb: np.ndarray) -> HpStoplightReading:
    """Classify bright HP-number pixels using the saved stoplight profile."""
    profile = load_hp_stoplight_profile()
