...
def test_preferences_default_to_auto_resize(tmp_path) -> None:
    ...
    assert values == {"auto_resize": True, "zoom_percent": 100}


def test_preferences_are_saved_and_zoom_is_bounded(tmp_path) -> None:
    path = tmp_path / "vision.json"
    save_preferences({"auto_resize": False, "zoom_percent": 500}, path)

    assert load_preferences(path) == {"auto_resize": False, "zoom_percent": 100}
