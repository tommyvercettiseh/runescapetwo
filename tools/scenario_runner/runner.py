...
def _validate_if(value: Any, *, depth: int) -> None:
    ...
    if condition_key == "definition":
        _parse_definition_condition(value[condition_key])
    else:
        _parse_image_condition(value[condition_key])
...
class ScenarioRunner:
    ...
    def run(self, data: dict[str, Any]) -> ScenarioResult:
        validate_scenario(data)
        name = str(data["name"])
        ...
        try:
            ...
            return ScenarioResult(stop.success, name, self.executed_steps, stop.message)

        trace("[SCENARIO] completed")
        return ScenarioResult(True, name, self.executed_steps, "Scenario completed.")
    ...
    def _run_if(self, value: dict[str, Any], *, depth: int) -> None:
        if "definition" in value:
            ...
            result = bool(entry.function(self.bot_id))
    ...
    def _run_action(self, value: Any) -> None:
        ...
        trace(f"[ACTION RESULT] {name}: {format_result(result).replace(chr(10), ' | ')}")
...
def run_scenario(
    ...
    *,
    ...
    return ScenarioRunner(bot_id=resolved_bot_id, dry_run=dry_run).run(data)


def run_scenario_with_console_trace(
    path: str | Path,
    *,
    bot_id: int | None = None,
    dry_run: bool = False,
) -> ScenarioResult:
    with capture_action_trace(print):
        return run_scenario(path, bot_id=bot_id, dry_run=dry_run)
...
__all__ = [
    ...
    "load_scenario",
    "run_scenario",
    "run_scenario_with_console_trace",
    "validate_scenario",
]
